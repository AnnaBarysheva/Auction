<?php
require __DIR__."/Style_dropdown.php";
...
if ($isSeller) {
    ...
    $queryExecutionTime = ($endQueryTime - $startQueryTime) * 1000;
    echo "Суммарное время выполнения запроса и сортировки: " . number_format($queryExecutionTime, 6) . " мс<br>";
    ...
    // $paintings = []; // Массив для хранения данных
    ...
    //         // Рассчитываем вес для каждой картины
    //         $styleWeight = $weights['style'] ?? 1;
    //         $requestWeight = $weights['has_request'] ?? 1;
    //         $creationYearWeight = $weights['creation_year'] ?? 1;
    
    //         $painting['total_weight'] =
    //             (in_array($painting['id_style'], $stylePreferences) ? 1 : 0) * $styleWeight +
    //             (in_array($painting['id_painting'], $userRequests) ? 1 : 0) * $requestWeight +
    //             ($painting['creation_year'] * $creationYearWeight);
    //     }
    
    //     // Сортировка массива по 'total_weight' в порядке убывания
    //     usort($paintings, function ($a, $b) {
    //         return $b['total_weight'] <=> $a['total_weight'];
    //     });
    //     $endTotalTime = microtime(true);
    //     $totalExecutionTime = ($endTotalTime - $startTotalTime) * 1000;

    //     // Вывод общего времени выполнения
    //     echo "Суммарное время выполнения запроса и сортировки: " . number_format($totalExecutionTime, 2) . " мс<br>";
...
//     return $dropdown;
// }
// Создание объекта класса painting_details
$styleDropdown = new Style_dropdown();
...
<div id="editModal" class="modal">
    <div class="modal-content">
        ...
        <form id="editForm" action="update_painting.php" method="POST">
            ...
            <?= $styleDropdown->createDropdownFromArray($stylesArray, 'editStyle', 'Выберите стиль', 'modal-input') ?>

            <label for="editMaterial">Материал:</label>
            <?= $styleDropdown->createDropdownFromArray($materialsArray, 'editMaterial', 'Выберите материал', 'modal-input') ?>
        ...
        <form id="addForm" action="add_painting.php" method="POST" enctype="multipart/form-data">
            <div class="input-container">
                <div class="input-column">
                    ...
                    <!-- <?=$styleDropdown-> createDropdownFromArray($stylesArray, 'styles', 'Выберите стиль') ?> -->    
                    <!-- <label for="materialDropdown">Материал:</label> -->
                    <!-- <?= createDropdown($materialsResult, 'materials', 'Выберите материал') ?> -->

                        <label for="styleDropdown">Стиль:</label>
                    <?= $styleDropdown->createDropdownFromArray($stylesArray, 'styles', 'Выберите стиль', 'select-dropdown') ?>

                    <label for="materialDropdown">Материал:</label>
                    <?=$styleDropdown-> createDropdownFromArray($materialsArray, 'materials', 'Выберите материал', 'select-dropdown') ?>
