...
<?php
require __DIR__."/Paintings_DetailsTest.php";
use App\Paintings_DetailsTest;

session_start();

// class painting_details
// {
...
$paintingDetails = new Paintings_DetailsTest();

// Вызов метода для получения информации о картине
$painting = $paintingDetails->getPaintingDetails($link, $id_painting);

if ($painting) {
    // Данные картины успешно получены
    if (isset($_SESSION['user_id'])) {
        $id_user = $_SESSION['user_id'];
        $id_style = $painting['id_style'];

        // Проверка, есть ли уже запись в таблице UserActivity
